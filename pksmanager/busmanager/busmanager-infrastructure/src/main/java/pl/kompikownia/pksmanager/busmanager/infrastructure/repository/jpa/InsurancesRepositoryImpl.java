...
public class InsurancesRepositoryImpl implements InsurancesRepository {
    ...
    public List<InsurancesProjection> findAll() {
        ...
        return query.from(QInsurancesEntity.insurancesEntity).orderBy(QInsurancesEntity.insurancesEntity.id.asc()).fetch()
                .stream()
    ...
    public List<InsurancesProjection> findAllByBusId(Long id) {
        JPAQuery<InsurancesEntity> query = new JPAQuery<>(em);
        return query.from(QInsurancesEntity.insurancesEntity).orderBy(QInsurancesEntity.insurancesEntity.id.asc())
                .where(QInsurancesEntity.insurancesEntity.bus.id.eq(id))
                .fetch()
                .stream()
                .map(InsurancesEntity::toProjection)
                .collect(Collectors.toList());
