...
import static pl.kompikownia.pksmanager.busmanager.infrastructure.entity.QFuelEntity.fuelEntity;
...
public class FuelRepositoryImpl  implements FuelRepository {
   ...
   private EntityManager em;
    ...
    public List<FuelProjection> findAll() {
        ...
        return query.from(fuelEntity).orderBy(fuelEntity.id.asc())
                .fetch()
                .stream()
    ...
    public void deleteById(Long id) {
        JPADeleteClause deleteClause = new JPADeleteClause(em,fuelEntity);
        deleteClause.where(fuelEntity.id.eq(id)).execute();
